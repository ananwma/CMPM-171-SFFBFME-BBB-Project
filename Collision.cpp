...
void Collision::move_left(Player &p1, Player &p2, AssetManager &current_screen) {
	if (((p1.character->sprite.getPosition().x) <= (p2.character->sprite.getPosition().x + p2.getSpriteWidth())) && (p1.getSide() == LEFT) && ((p1.character->sprite.getPosition().y + p1.getSpriteHeight() / 2) >= (p2.character->sprite.getPosition().y - p2.getSpriteHeight() / 2))) {
	}
	else {
